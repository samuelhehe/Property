 ...
 * 显示小区信息坐标, 店家地址坐标
 */
public class ShowAddressActivity extends HXBaseActivity implements OnGetGeoCoderResultListener {
...
//                Toast.LENGTH_LONG).show();
    ...
    private class myOnGetGeoCoderResultListener implements OnGetGeoCoderResultListener{
        ...
        public void onGetReverseGeoCodeResult(ReverseGeoCodeResult result) {
            if (result == null || result.error != SearchResult.ERRORNO.NO_ERROR) {
                Toast.makeText(ShowAddressActivity.this, "抱歉，未能找到结果", Toast.LENGTH_LONG)
                        .show();
                return;
            }
//        mBaiduMap.clear();
...
//                        .fromResource(R.drawable.icon_marka)));
            Log.i("debbug", "result="+result.getLocation().toString());
            Log.i("debbug", "address="+result.getAddress());
            View frameLayout = View.inflate(ShowAddressActivity.this, R.layout.map_marker_view, null);
            TextView t1 = (TextView) frameLayout.findViewById(R.id.tv_txt);
            t1.setText(PreferencesUtil.getCommityName(ShowAddressActivity.this));
