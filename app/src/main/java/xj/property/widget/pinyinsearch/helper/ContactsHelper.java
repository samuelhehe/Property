package xj.property.widget.pinyinsearch.helper;
...
import xj.property.model.PinyinBaseUnit;
import xj.property.model.PinyinUnit;
import xj.property.model.PinyinUtil;
import xj.property.model.QwertyMatchPinyinUnits;
import xj.property.model.T9MatchPinyinUnits;
...
public class ContactsHelper {
    ...
    private AsyncTask<Void, Void, List<Contacts>> mLoadTask = null;
...
//		return true;
//	}
    public boolean startLoadContacts(final List<InvitedNeighborBean.InfoEntity> invitedEntities) {
        if (true == isSearching()) {
            return false;
        }
        ...
        mLoadTask = new AsyncTask<Void, Void, List<Contacts>>() {

            @Override
            protected List<Contacts> doInBackground(Void... params) {
                return fiterAndSort(invitedEntities, loadContacts(mContext));
            }

            private List<Contacts> fiterAndSort(List<InvitedNeighborBean.InfoEntity> invitedEntities, List<Contacts> contactses) {

                if (invitedEntities == null || invitedEntities.isEmpty() || invitedEntities.size() <= 0) {
                    return contactses;
                }
                if (contactses == null || contactses.isEmpty() || contactses.size() <= 0) {
                    return contactses;
                }
                for (InvitedNeighborBean.InfoEntity info : invitedEntities) {
                    for (Contacts contact : contactses) {
                        if (TextUtils.equals(info.getPhone(), contact.getPhoneNumber())) {
                            contact.setRegistAvatar(info.getRegistAvatar());
                            contact.setRegistNickname(info.getRegistNickname());
                            contact.setRegistEmobId(info.getRegistEmobId());
                        }
                    }
                }
                return contactses;
            ...
            if (true == T9MatchPinyinUnits.matchPinyinUnits(pinyinUnits, name, search, chineseKeyWord)) {// search by NamePinyinUnits;
                ...
                for (currentContacts = mBaseContacts.get(i); null != currentContacts; currentContacts = currentContacts.getNextContacts()) {
                    if (currentContacts.getPhoneNumber().contains(search)) {// search by phone number
                        currentContacts.setSearchByType(Contacts.SearchByType.SearchByPhoneNumber);
    ...
    private void initContactsHelper() {
        ...
        mContext = XjApplication.getInstance();
