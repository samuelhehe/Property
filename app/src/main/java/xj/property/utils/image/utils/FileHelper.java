	...
	public static boolean deleteDirectory(String filePath)
	{
		...
		if (file.isDirectory())
		{
			...
			for (int i = 0; i < list.length; i++)
			{
				...
				// File to = new File(list[i].getAbsolutePath() +
				// System.currentTimeMillis());
				if (list[i].isDirectory())
				{
					// list[i].renameTo(to);
					deleteDirectory(list[i].getAbsolutePath());
					// deleteDirectory(to.getAbsolutePath());
				} else
				{
					// list[i].renameTo(to);
					list[i].delete();
		...
		// the sub dir 'file' should not be deleted
		// or EBUSY error will happen next time write to file dir
