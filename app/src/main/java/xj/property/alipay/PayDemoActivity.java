...
public class PayDemoActivity extends HXBaseActivity {
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        ...
                        intent.setClass(PayDemoActivity.this, PayPreActivity.class);
...
//				+ getSignType();
    ...
    private void getOrderInfo() {
        ...
        Callback<OrderInfoBean> callback = new Callback<OrderInfoBean>() {
            @Override
            public void success(OrderInfoBean bean, retrofit.client.Response response) {
                mLdDialog.dismiss();
                orderInfo = bean.info;
                showToast("获取订单成功");
            }
        ...
        request.subject = "帮帮订单";
        request.orderId = getIntent().getStringExtra(Config.EXPKey_serial);
        request.userBonusId = getIntent().getIntExtra("useBonusId", 0);
        Log.i("onion","userBonusId:  "+getIntent().getIntExtra("useBonusId", 0));
       // request.price = "20";

        double payPrice = getIntent().getDoubleExtra(Config.EXPKey_orginPrice,0);

        request.price= Arith.round(payPrice,2)+"";

        Log.i("onion  request.price ",Arith.round(payPrice,2)+"");

        //// 帮币的使用数量
        request.bonusCoinCount=getIntent().getIntExtra("bonusCoinCount",0);

        Log.i("onion",getIntent().getIntExtra("bonusCoinCount",0)+"");


        service.getOrderInfo(StrUtils.string2md5(Config.BANGBANG_TAG+ StrUtils.dataHeader(request)),request, bean.getCommunityId(), bean.getEmobId(), callback);
    }
