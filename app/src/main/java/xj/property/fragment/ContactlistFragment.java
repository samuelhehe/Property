...
import xj.property.adapter.ContactAdapter;
...
import xj.property.activity.HXBaseActivity.AddContactActivity;
...
//                    if(contact.getSort().equals("5")){
//                        startActivity(new Intent(getActivity(), ChatActivity.class).putExtra("userId", contact.getEmobId()));
//                    }else if(contact.getSort().equals("1")) {
//
//                    }
                    Log.i("onion",contact.toString());
                    // demo中直接进入聊天页面，实际一般是进入用户详情页
					startActivity(new Intent(getActivity(), ChatActivity.class).putExtra("userId", contact.getUsername()));
				}
			}
		});
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					XjApplication.getInstance().getContactList().remove(tobeDeleteUser.getUsername());
	...
	private void getContactList() {
		contactList.clear();
		//获取本地好友列表
		Map<String, User> users = XjApplication.getInstance().getContactList();
