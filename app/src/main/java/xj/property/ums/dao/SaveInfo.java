...
public void run() {
	...
	try {
		if(Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)&&CommonUtil.checkPermissions(context, "android.permission.WRITE_EXTERNAL_STORAGE")){
            ...
            Log.i("onion","开始存异常信息"+file.getAbsolutePath());
			...
			if(sb.length()!=0){
				 existJSON = new JSONObject(sb.toString());
					...
					FileOutputStream fileOutputStream = new FileOutputStream(Environment.getExternalStorageDirectory()+"/mobclick_agent_cached_"+context.getPackageName(),true);
